...
public:
    ...
    void swapRows(int row1, int row2) {
        if (row1 < row && row2 < row) {
            int *temp = new int[row];   //동적할당으로 생성해야함
            for (int i = 0; i < col; i++) {
    ...
    int determinantFinder(int n) {
        int determinant = 0;
        int matrix[10][10];
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                matrix[i][j] = getij(i, j);
            }
        }
        if (n == 2) {
            ...
            for (int p = 0; p < n; p++) {
                ...
                determinant = determinant + matrix[0][p] * pow(-1, p) * determinantFinder(n - 1);
            }
        }
        return determinant;
    }
...
int main() {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            int col = a.getcol();    
            if(row == col){
                int det = a.determinantFinder(row);
